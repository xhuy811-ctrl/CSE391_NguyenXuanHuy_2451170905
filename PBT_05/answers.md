...
## SCSS refactor (PBT_05)

I created an SCSS structure under `scss/` and compiled to `style.css` at the project root.

- Files added:
  - `scss/_variables.scss`
  - `scss/_mixins.scss`
  - `scss/_components.scss`
  - `scss/style.scss` (main)
  - `style.css` (compiled output)

- SCSS compile command (run from project root):

```bash
sass scss/style.scss style.css --no-source-map --style=expanded
```

Variables used consistently, nesting and parent selectors included, and practical mixins for responsiveness and shadows were added.
